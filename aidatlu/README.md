...
### Internal Trigger Generation (`internal_trigger_rate`)
...
Optional configuration, specific DUT interface ignores busy signals (`ignore_busy`).
...
This includes trigger input thresholds, `trigger_logic`, trigger `polarity` and trigger signal shaping (`stretch` and `delay`).

The `threshold` for each trigger input can be tuned individually between [-1.3; 1.3] V.

Another setting controls the trigger input logic (`trigger_input_logic`).
...
Each trigger input can trigger on a rising or falling edge. Trigger `polarity` is set using a string or boolean,
...
### Clock LEMO (`enable_clock_lemo_output`)
The clock LEMO setting enables or disables the clock LEMO output.
Set this to 'True' or 'False'.

### PMT Power (`pmt_power`)
Set the PMT `control_voltage`. The possible range is between [0; 1] V.

### Data Handling and Online Monitor
Two settings concern the interpretation of raw data (`save_data`).
At last, the ZMQ connection for the [online_monitor](https://github.com/SiLab-Bonn/online_monitor) (`zmq_connection`) can be configured.

### Stop Conditions
Two optional stop conditions (`max_trigger_number` and `timeout`) can be set in tlu_configuration.yaml.
